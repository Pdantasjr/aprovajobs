<template>
    <div class="rounded-full flex w-full my-4">
        ...
        <span :class="[baseTextColor, baseBg]" class="text-center flex px-4 py-2  items-center justify-center border rounded-r-full w-full">
            <span class="mx-2 text-light font-semibold">Descrição do status</span>
        </span>
    </div>
</template>
...
export default {
    ...
    computed: {
        baseBg() {
            if (this.status === 0) {
                return 'border-dark/30 dark:border-light/30'
        ...
        baseTextColor() {
            if (this.status === 0) {
                return 'text-dark/30 dark:text-light/30'
            } else if (this.status === 1) {
                return 'text-info'
            } else if (this.status === 2) {
                return 'text-danger'
